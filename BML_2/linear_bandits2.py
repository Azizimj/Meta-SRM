...
BAI = True  # indicating that it is BAI, e.g. no new context
BAI = False

algos_BML = ['mts', 'oracle', 'misspecified']

class LinearBandit():

    def __init__(self, d, K, H, prior=None, noise=False, seed=None, arm_dist='Gaus', weight_dist='MulGaus'):
        self.d = d
        self.K = K
        self.H = H
        self.noise = noise
...
def one_iter_BML(Args, mean, cov, iter_cnt=0, arm_dist='Gaus', weight_dist='MulGaus'):
...
if __name__=='__main__':
    ...
    parser.add_argument('--alg', action='store', choices=algos_BML, default='oracle')
...
#     (mean,cov) = INSTANCES['block'](Args.d)
    ...
    if parr:
        ...
        poolobjs = [pool.apply_async(one_iter_BML, args=[Args_d, mean, cov, i], callback=collect_result)
        ...
        for i in range(Args.iters):
            ...
            (_, vec, svec, me, ce, _, _, _) = one_iter_BML(Args_d, mean, cov, i)
